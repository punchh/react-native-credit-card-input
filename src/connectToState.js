...
export default function connectToState(CreditCardInput) {
	class StateConnection extends Component {
		...
		setValues = values => {
			const { requiresAlphanumericPostalCode } = this.props;
			...
			const formattedValues = new CCFieldFormatter(displayedFields).formatValues(
				newValues,
				requiresAlphanumericPostalCode
			);
			const validation = new CCFieldValidator(displayedFields, this.props.validatePostalCode).validateValues(
				formattedValues
			);
